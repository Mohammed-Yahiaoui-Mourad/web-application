...
export default function CreateHospitalAdmin() {
  ...
  const [hopitals, setHopitals] = useState<any[]>([])
  ...
  async function handleSubmit(e: any) {
    ...
    try {
      await api.post('/api/admin/create-hospital-admin', form)
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <p className="mb-6 text-sm text-gray-600">
        {hopitals.length} hôpitaux chargés. Choisissez l&apos;établissement à administrer.
      </p>
        ...
        <div>
          ...
          <HospitalSelect
            hopitals={hopitals.filter(
              (h: any) => !filterRegion.trim() || h.region?.toLowerCase().includes(filterRegion.trim().toLowerCase())
            )}
            value={form.hopital_id}
            onChange={(hopital_id: any) => setForm((f) => ({ ...f, hopital_id }))}
          />
        </div>
